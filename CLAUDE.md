...
**Current state**: Fully running on 64 GB RAM (motherboard upgrade applied 2026-05-17, ~58.9 GB usable). Security hardening applied 2026-05-14; Hagga Basin travel fixed 2026-05-15. The **Live** world `Ixware` (`sh-db3533a2d5a25fb-silakw`, namespace `funcom-seabass-sh-db3533a2d5a25fb-silakw`) is the active capsule; a second Live world `SlackSalusa` (`sh-db3533a2d5a25fb-xfrcer`) exists as a parked cold capsule (separate-character world — see `MULTI-WORLD.md`); the PTC capsule `Slackware-Arrakis` (`sh-db3533a2d5a25fb-xyyxbx`) is configured but cold. Only one world is online at a time (hot-swap); swapping back into a parked world currently needs a manual restore (`SWAP-RESTORE-DESIGN.md`). Survival_1, Overmap, DeepDesert_1, SH_Arrakeen, and SH_HarkoVillage run continuously via director persistence (`MinServers=1` for DD/social hubs). Conan Exiles Enhanced co-tenant uses ~9.5 GB RSS. Total swap: 62 GB headroom (zram + dune-vg SSD + sdc1). VPA recommender live (Off mode, memory only). FLS token expires 2027-06-22 — rotate by 2027-05-23.
